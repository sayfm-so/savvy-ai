  ...
  create() {
    ...
    lines.forEach((line,i) => {
      const t = this.add.text(W/2, H/2-90+i*34, line, {
        fontFamily:'Courier New', fontSize:'28px', color:'#00ffcc',
...
class WorldScene extends Phaser.Scene {
  ...
  _buildOffice() {
    ...
    this.rooms.forEach(r => {
      ...
      this.add.text(r.x+8,r.y+7,r.label,{
        fontFamily:'Courier New', fontSize:'24px', color:r.labelColor,
      }).setAlpha(0.9);
    });
  ...
  _buildDecorations() {
    ...
    this.add.text(HUD_W+470,487,'[RECEPTION]',{
      fontFamily:'Courier New', fontSize:'24px', color:'#4fa3ff',
    }).setOrigin(0.5);
  ...
  _drawRobotIcon(g, x, y) {
    ...
    this.add.text(x+12,y+36,'G1',{
      fontFamily:'Courier New', fontSize:'14px', color:'#00ffcc',
    }).setOrigin(0.5,0);
  }
  ...
  _buildActors() {
    ...
    defs.forEach(d => {
      ...
      const label = this.add.text(d.x, d.y - sprite.displayHeight - 4, d.name, {
        fontFamily:'Courier New', fontSize:'17px',
  ...
  _buildHUD() {
    ...
    this.add.text(lx+16,ly,'SAVVY AI',{
      fontFamily:'Courier New',fontSize:'24px',color:'#0088ff',fontWeight:'bold',
    }).setOrigin(0,0.5).setDepth(51);

    // Agent status
    this.add.text(10,55,'AGENT STATUS',{
      fontFamily:'Courier New',fontSize:'17px',color:'#336655',
    }).setDepth(51);
    ...
    agentDefs.forEach((a,i) => {
      this.add.text(10,76+i*28,`■ ${a.name}`,{
        fontFamily:'Courier New',fontSize:'17px',color:a.color,
      }).setDepth(51);
      const st = this.add.text(165,76+i*28,'IDLE',{
        fontFamily:'Courier New',fontSize:'28px',color:'#336655',
      }).setDepth(51);
    ...
    for(let i=0;i<12;i++){
      const ft = this.add.text(10,306+i*32,'',{
        fontFamily:'Courier New',fontSize:'17px',color:'#226644',
    ...
    this.statusBar = this.add.text(HUD_W+14,H-15,
      '> SEOUL HQ — LOCAL INTELLIGENCE ONLINE',{
        fontFamily:'Courier New',fontSize:'24px',color:'#0050ff',
      }).setOrigin(0,0.5).setDepth(51);
    this.clockText = this.add.text(W-12,H-15,'',{
      fontFamily:'Courier New',fontSize:'24px',color:'#336655',
    ...
    if(a.state==='WALKING'){
      ...
      a.bubble = this.add.text(
        ...
        thoughts[Math.floor(Math.random()*thoughts.length)],{
          fontFamily:'Courier New', fontSize:'28px', color:'#ffee44',
